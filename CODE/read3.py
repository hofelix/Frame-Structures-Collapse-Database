...
input = np.genfromtxt('C:/Users/USER/Desktop/OUTPUT/OUTPUT3/all.csv')
input = np.reshape(input,(1000,21))
...
for i in range(0,21):
    ...
    #print(nor[21])
    data.append(nor)
data = np.reshape(data,(1000,21))
#data = list(data)
#fo.write( str(data) )
#print(data[:,0])
disout = 0.175*data[:,0]+0.175*data[:,1]+0.15*data[:,2]+0.175*data[:,3]+0.175*data[:,4]+0.15*data[:,5]
disin = data[:,6]
cosout = 0.175*data[:,7]+0.175*data[:,8]+0.15*data[:,9]+0.175*data[:,10]+0.175*data[:,11]+0.15*data[:,12]
cosin = data[:,13]
curout = 0.175*data[:,14]+0.175*data[:,15]+0.15*data[:,16]+0.175*data[:,17]+0.175*data[:,18]+0.15*data[:,19]
curin = data[:,20]
